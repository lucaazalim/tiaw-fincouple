...
$('#cadastrar').click(function (event) {
    ...
    for (let i = 0; i < usuarios.length; i++) {
        ...
        if (!usuario_regex.test(usuario)) {
            alertar(`O usuário <b>${usuario}</b> é inválido. Insira um usuário de 4 a 16 caracteres, utilizando apenas letras, números e "_".`, 'danger');
            return;
        }

        if (Usuarios.buscarPorUsuario(usuario)) {
            alertar(`O usuário <b>${usuario}</b> já existe!`, 'danger');
            return;
        }

        usuarios[i] = new Usuarios.Usuario(nome, sobrenome, usuario, senha, casal.id);
